...
class DefaultController extends Controller {
	...
	async create(req, res, next) {
		// If body is empty
		if (isEmptyObject(req.body)) return res.status(400).json({ message: 'Request body is empty' });
	...
	async read(req, res, next) {
		...
		const limit = req.queryParsed.limit,
			...
			match = req.queryParsed.match,
			include = req.queryParsed.include;
		try {
	...
	async update(req, res, next) {
		const id = req.params.id;
		const db = Controller.api.db.connection;
		if (!id) throw new Controller.errors.BadRequestError('Resource id is required');
		if (!req.body) throw new Controller.errors.BadRequestError('Request body is empty');
		...
		try {
			const response = await db.collection(this._model.name).findOneAndUpdate({ _id: new ObjectId(id) }, update, options)
			const doc = response.value
			if (!doc) throw new Controller.errors.NotFoundError('Record not found');
