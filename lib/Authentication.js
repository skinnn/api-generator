 ...
 * 
 * 1. First get endpoint & operation which user is trying to access
 ...
 * 		- Check if user is authorized to access the requested CRUD on endpoint
 */
...
class Authentication extends Controller {
	...
	static async checkPermissions(resource, operation, endpoint, userRoles) {
		...
		const hasRole = helpers.haveCommonElements(endpoint._schema.access[operation].roles, userRoles)
		if (hasRole) {
			authorized = true
